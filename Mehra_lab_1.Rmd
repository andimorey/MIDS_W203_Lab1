...
> Since all the assumptions are satisfied, we will not go ahead with using the unpaired t-test.
...
> There are no Null Values for the variable russia16 or muellerinv of independents
...
> The distribution for muellerinv for independent voters seems symmetric about 4 and the skewness level is at a low (skewness is between -0.5 and 0.5,).
...
> 1. Since we are comparing means of two samples, it will be a two-sample test. 
> 2. Since the variable we are comparing is an ordinal variable, parametric tests cannot be used.
> 3. Since the two variables we are looking at are answered by the different people and there is no dependency we can take advantage of, we will do a unpaired test. 
> With all these above reasons, we narrow our choice of test down to the Wilcoxon rank-sum test.

> Assumptions to be able to use the Wilcoxon rank-sum test are:
> 1. Ordinal variable
> 2. Data is paired and drawn from i.i.d samples

> Since all the assumptions are satisfied, we will not go ahead with using the Wilcoxon rank-sum test.
...
> This shows there is a higher ratio of independent voters who approve of the Mueller Investigation. We can deduce that they must think the federal investigations of Russian election interference were not baseless. Therefore, there is practical significance in rejecting the null hypothesis.   
